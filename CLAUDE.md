...
- **macOS app**: `tools/build_macapp.sh [--install]` packages the GUI into a
  standalone `VoiceLab.app` via PyInstaller (bundles Python + PySide6 + pyserial +
  `galetsynth`; runs without the repo/.venv). `--install` copies it to
  `/Applications`. Rebuild after any GUI change with the same command. The icon is
  generated by `tools/make_icon.py` → `tools/voicelab.icns` (committed; the build
  passes it via `--icon`). PyInstaller work files go to `build-macapp/` (git-
  ignored) — kept out of the firmware `build/` dir.
