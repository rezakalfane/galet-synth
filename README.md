...
| `tools/build_macapp.sh` | Package the tuner GUI as a standalone macOS `VoiceLab.app` (PyInstaller) |
| `tools/make_icon.py` | Generates the app icon (`tools/voicelab.icns`) — synth-voice waveform |
...
### Package the tuner as a macOS app

Build the GUI into a standalone **`VoiceLab.app`** — no terminal needed to launch it:

```bash
tools/build_macapp.sh            # → build-macapp/dist/VoiceLab.app
tools/build_macapp.sh --install  # …and copy it to /Applications
```

PyInstaller bundles its own Python + PySide6 + pyserial + the `galetsynth`
package, so the app runs independently of this repo and the `.venv`. The build is
self-contained (~100 MB); rebuild after any GUI change with the same command. The
app icon (a synth-voice waveform) is generated by `tools/make_icon.py` into
`tools/voicelab.icns` — edit the colours/waveform there and rerun it to restyle.
PyInstaller's work files land in `build-macapp/` (git-ignored). The app is
ad-hoc-signed and built locally (no quarantine), so it just opens; only if you
copy it to *another* Mac would Gatekeeper ask you to right-click → Open once.
